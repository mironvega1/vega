...
import { DataIntake } from '@/components/command-center/DataIntake'
...
function CommandCenterView() {
  ...
  const setupSteps = [
    { label: 'Müşteri', count: data.customers.length, done: data.customers.length > 0, help: 'Kiminle çalışıyorsun?' },
    { label: 'Portföy', count: data.portfolios.length, done: data.portfolios.length > 0, help: 'Hangi mülk takipte?' },
    { label: 'İşlem', count: data.deals.length, done: data.deals.length > 0, help: 'Müşteri + portföy bağlantısı' },
    { label: 'Temas', count: data.interactions.length, done: data.interactions.length > 0, help: 'Arama, mesaj, görüşme' },
  ]
  const nextStep = setupSteps.find((step) => !step.done)
  const recentRecords = [
    ...data.customers.slice(0, 3).map((item) => ({ id: `customer-${item.id}`, type: 'Müşteri', title: item.name, detail: `${item.segment} · aciliyet ${item.urgency}/100` })),
    ...data.portfolios.slice(0, 3).map((item) => ({ id: `portfolio-${item.id}`, type: 'Portföy', title: item.title, detail: `${item.type}${item.district ? ` · ${item.district}` : ''} · ${money(item.listPrice)}` })),
    ...data.deals.slice(0, 3).map((item) => ({ id: `deal-${item.id}`, type: 'İşlem', title: item.title, detail: `${item.status === 'active' ? 'Aktif' : item.status === 'won' ? 'Kazanıldı' : 'Kaybedildi'} · ${money(item.expectedRevenue)}` })),
  ].slice(0, 6)
        ...
        <section className="cc-col cc-center">
          <section className="cc-hero">
            <div>
              ...
              <h1>Eklediğin kayıtlar burada canlı takip edilir.</h1>
              <p>
                Önce müşteri, sonra portföy, sonra işlem ekle. İşlem oluşunca tahmin, risk ve
                yapılacak iş önerileri görünür şekilde değişir.
              </p>
            </div>
          ...
          <Panel kicker="NASIL ÇALIŞIR?" title={nextStep ? `Sıradaki adım: ${nextStep.label}` : 'Akış tamamlandı'} meta={`${setupSteps.filter((step) => step.done).length}/4 adım`}>
            <div className="cc-setup-grid">
              {setupSteps.map((step, index) => (
                <article key={step.label} className={step.done ? 'done' : ''}>
                  <b>{index + 1}</b>
                  <strong>{step.label}</strong>
                  <span>{step.count} kayıt</span>
                  <p>{step.help}</p>
                </article>
              ))}
            </div>
            <div className="cc-comment">
              {nextStep
                ? `${nextStep.label} adımı eksik. Bu adım tamamlanınca ekranın ilgili bölümleri çalışmaya başlar.`
                : 'Temel kayıt akışı hazır. Temas ve fiyat güncellemeleri geldikçe öneriler daha netleşir.'}
            </div>
          </Panel>

          <Panel kicker="SON EKLENENLER" title="Kayıtlar Sisteme Geldi mi?" meta={`${recentRecords.length} kayıt`}>
            {recentRecords.length ? (
              ...
              <Empty text="Henüz kayıt yok. Sol panelden müşteri ekleyince ilk kayıt burada hemen görünür." />
            )}
          </Panel>
          ...
          <Panel kicker="ANALİZ AKIŞI" title="Analizlerden Gelen Sonuçlar" meta={`${data.analysisResults.length} analiz`}>
...
.cc-setup-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.cc-setup-grid article{border:1px solid #242424;background:#090909;border-radius:8px;padding:12px}.cc-setup-grid article.done{border-color:rgba(34,197,94,.36);background:rgba(34,197,94,.045)}.cc-setup-grid b{display:inline-flex;width:22px;height:22px;border-radius:6px;align-items:center;justify-content:center;background:rgba(255,215,0,.08);border:1px solid rgba(255,215,0,.2);color:#FFD700;font-size:11px}.cc-setup-grid strong{display:block;color:#f0f0f0;font-size:13px;margin-top:10px}.cc-setup-grid span{display:block;color:#FFD700;font-size:12px;font-weight:800;margin-top:4px}.cc-setup-grid p{margin:6px 0 0;color:#888;font-size:11px;line-height:1.4}
...
.cc-goals{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}.cc-goals label{display:grid;gap:5px;color:#888;font-size:10px;letter-spacing:.7px}.cc-goals input,.cc-goals select,.cc-data-console textarea,.cc-form input,.cc-form select{width:100%;box-sizing:border-box;background:#090909;border:1px solid #252525;border-radius:7px;color:#ededed;padding:9px;outline:none}.cc-plan-summary{border:1px solid #222;background:#090909;border-radius:8px;padding:10px;color:#aaa;font-size:12px;margin-bottom:9px}.cc-stack{display:grid;gap:8px}.cc-action{display:flex;justify-content:space-between;gap:10px;border:1px solid #222;border-radius:8px;background:#090909;padding:10px}.cc-action p{margin:5px 0 0;color:#888;font-size:12px}.cc-action span{color:#FFD700;font-weight:800}.cc-blocked{margin-top:12px}.cc-blocked h3{font-size:13px;margin:0 0 8px}.cc-line{display:flex;justify-content:space-between;gap:8px;color:#888;border-bottom:1px solid #171717;padding:7px 0;font-size:12px}
.cc-intake-help{border:1px solid rgba(255,215,0,.2);background:rgba(255,215,0,.045);border-radius:8px;padding:10px 11px;color:#d7c85a;font-size:11px;line-height:1.5;margin-bottom:10px}.cc-intake-steps{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:5px;margin-bottom:10px}.cc-intake-steps div{border:1px solid #202020;background:#090909;border-radius:7px;padding:7px 4px;text-align:center}.cc-intake-steps b{display:block;color:#FFD700;font-size:11px}.cc-intake-steps span{display:block;color:#888;font-size:9px;margin-top:3px}.cc-tabs{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:5px;margin-bottom:10px}.cc-tabs button{border:1px solid #242424;background:#101010;color:#777;border-radius:7px;padding:8px 4px;font-size:10px;cursor:pointer}.cc-tabs button.active{border-color:rgba(255,215,0,.38);background:rgba(255,215,0,.08);color:#FFD700}.cc-form{display:grid;gap:8px}.cc-form label{display:grid;gap:5px;color:#888;font-size:10px;letter-spacing:.6px}.cc-form button{border:0;border-radius:7px;background:#FFD700;color:#050505;font-weight:850;padding:10px 12px;cursor:pointer}.cc-two{display:grid;grid-template-columns:1fr 1fr;gap:8px}.cc-form-note{margin-top:9px;border:1px solid rgba(34,197,94,.25);background:rgba(34,197,94,.06);color:#9ee6b7;border-radius:7px;padding:9px;font-size:11px;line-height:1.45}
...
@media(max-width:1180px){.cc-shell{grid-template-columns:1fr}.cc-left,.cc-right{position:static;max-height:none}.cc-hero,.cc-metrics,.cc-forecast-grid,.cc-setup-grid{grid-template-columns:1fr 1fr}}@media(max-width:720px){.cc-header{height:auto;align-items:flex-start;flex-direction:column;padding:14px}.cc-hero,.cc-metrics,.cc-forecast-grid,.cc-setup-grid,.cc-goals{grid-template-columns:1fr}.cc-shell{padding:8px}.cc-hero h1{font-size:28px}}
`
